NFPSarchive for MATLAB Production Server

Before you can use the functions packaged by the compiler, you need to do the following:
* Deploy NFPSarchive.ctf to a server instance
* Develop a client application to access the functions.

1. Deploy NFPSarchive.ctf

1.1 Prerequisites to Deployment
Before you can deploy NFPSarchive.ctf, you need a server instance to host it. The server 
instance must be configured to have access to MATLAB Runtime(R2023a).
* If you do not have a server instance created, see: 
  https://www.mathworks.com/help/mps/server/creating-a-server.html
* If you do not have MATLAB Runtime(R2023a) installed, download the installer from the 
  MATLAB Runtime page at: https://www.mathworks.com/products/compiler/mcr/index.html
* If you need to configure the server instance to use MATLAB Runtime(R2023a), see: 
  https://www.mathworks.com/help/mps/server/customize-the-configuration-file.html

1.2 NFPSarchive.ctf Deployment
To deploy NFPSarchive.ctf, copy it into the server instance's `auto_deploy` folder. The 
